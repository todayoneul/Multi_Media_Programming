...
TCHAR g_szFontName[190] = { DEFAULT_FONT_NAME }; 
...
HRESULT BlendApplicationImage(HWND hwndApp)
{
	...
	if (FAILED(hr))
	{
		Msg(TEXT("GetNativeVideoSize FAILED! hr = 0x%xWrWn"), hr);
			return hr;
	}
	//load the bitmap to alpha blend from the resource file
	HBITMAP hbm = LoadBitmap(ghInst, MAKEINTRESOURCE(IDB_BITMAP1)); // ¼öÁ¤
	...
	if (FAILED(hr)) {
		Msg(TEXT("SetAlphaBitmap FAILED! hr=0x%xWr#n#r#n%s\0"), hr, STR_VMR_DISPLAY_WARNING);
	}
...
HRESULT DisableTicker(DWORD dwFlags)
{
	...
	if (FAILED(hr))
		Msg(TEXT("GetAlphaBitmapParameters FAILED!	hr = 0x % xWrWn"), hr);
	if (dwFlags & MARK_DISABLE) {
		...
		if (g_dwTickerFlags & MARK_STATIC_IMAGE) hr = BlendApplicationImage(ghApp);
		else hr = BlendApplicationText(ghApp, g_szAppText);
...
void ResetBitmapPosition(void)
{
	...
	hr = pBMP->GetAlphaBitmapParameters(&bmpInfo); 
	if (FAILED(hr))
		Msg(TEXT("GetAlphaBitmapParameters FAILED!	hr = 0x%xWr\n"), hr);
...
HFONT SetTextFont(BOOL bShowDialog)
{
	...
	lstrcpy(g_szFontName, lf.lfFaceName);
...
HRESULT BlendApplicationText(HWND hwndApp, TCHAR* szNewText)
{
	...
	hr = pBMP->SetAlphaBitmap(&bmpInfo);
	if (FAILED(hr))
		Msg(TEXT("SetAlphaBitmap FAILED!	hr = 0x % xWrWnWrWn % s\0"), hr, STR_VMR_DISPLAY_WARNING);
