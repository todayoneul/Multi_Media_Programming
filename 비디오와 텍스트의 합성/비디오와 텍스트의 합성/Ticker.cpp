...
void Msg(TCHAR* szFormat, ...)
{
	...
	MessageBox(NULL, szBuffer, TEXT("VMRTicker Sample\0"), MB_OK); // \0추가
}
...
LRESULT CALLBACK WndMainProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_SET_FONT:
			...
			break;
		case ID_SET_TEXT:
			DialogBox(ghInst, MAKEINTRESOURCE(IDC_EDIT_TEXT), ghApp, (DLGPROC)TextDlgProc);
			break;
		//case ID_HELP_ABOUT:
...
HRESULT InitializeWindowlessVMR(IBaseFilter** ppVmr9)
{
	...
	*ppVmr9 = NULL;
	...
	else Msg(TEXT("Failed to create Vmr! hr=0x%x\r\n"), hr);
	return hr;
}
...
int PASCAL WinMain(HINSTANCE hInstC, HINSTANCE hInstP, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (lpCmdLine[0] != '\0') {
		USES_CONVERSION;
		strncpy_s(g_szFileName, A2T(lpCmdLine), NUMELMS(g_szFileName));//_tcsncpy
	}
	...
	wc.lpszMenuName = MAKEINTRESOURCE(IDR_MENU1); // 수정함
	wc.hCursor = LoadCursor(NULL, IDC_ARROW);
	wc.hIcon = LoadIcon(hInstC, MAKEINTRESOURCE(IDB_BITMAP1)); // 수정함
	...
	if (ghApp) {
		...
		if (g_szFileName[0] != 0)
			PostMessage(ghApp, WM_COMMAND, ID_FILE_CLOSECLIP, 0);
