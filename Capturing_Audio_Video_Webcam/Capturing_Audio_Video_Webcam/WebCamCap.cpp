...
int main(int argc, char* argv[])
{
	...
	hr = pCaptureGraph->SetOutputFileName(&MEDIASUBTYPE_Avi, L"C:\\MyWebcam.AVI", &pAVIWriter, &pSink);
	...
	hr = GetAudioInputFilter(&pAudioInputFilter, L"PC Camera"); // 뭘 찾아서 바꾸라는데 내장 마이크, 카메라의 이름을 L""에 넣어라.
	if (SUCCEEDED(hr)) hr = pGraph->AddFilter(pAudioInputFilter, L"Realtek(R) Audio");

	hr = GetVideoInputFilter(&pVideoInputFilter, L"Lebeca PC Camera"); // 그게 뭔진 모름.
	if (SUCCEEDED(hr)) hr = pGraph->AddFilter(pVideoInputFilter, L"LG Camera");
